...
const ASSIGNMENTS = [
  ...
  // ============================================================
  // DIRECTION A — SCORE GRID (students x assignments matrix)
  // ============================================================
  function renderA(){
    const root = $("#dirA"); root.innerHTML=""; root.style.position="relative";
    const VIEW_ASGS=ASSIGNMENTS.slice(0,20); // cap grid views to 20 most recent
    // context / setup bar
    const ctx = el("div","ctx");
    ctx.innerHTML =
      '<div class="field"><span class="lab">Assignments <span class="muted" style="font-weight:400;font-size:13px;">('+ASSIGNMENTS.length+' total · showing 20 most recent)</span></span><div class="chips">'+
        VIEW_ASGS.map(a=>'<span class="chip on">'+a.name+' <span class="x">×</span></span>').join('')+
        '<span class="chip add">+ add</span>'+
      '</div></div>';
    const pairs=[]; STUDENTS.forEach(s=>VIEW_ASGS.forEach(a=>pairs.push([s.id,a.id])));
    ctx.appendChild(progressEl(countDone(pairs),pairs.length,"scores"));
    root.appendChild(ctx);

    // grid panel
    const panel = el("div","rough"); panel.style.cssText="background:var(--panel);border-radius:12px;padding:6px 4px 10px;overflow:auto;";
    const table = el("table"); table.style.cssText="border-collapse:collapse;width:100%;font-family:'Kalam',cursive;";
    // head
    const thead = el("thead");
    let hr = '<tr><th style="text-align:left;padding:12px 14px;position:sticky;left:0;background:var(--panel);font-size:15px;min-width:150px;">Student</th>';
    VIEW_ASGS.forEach(a=>{ hr += '<th style="padding:12px 10px;font-size:15px;min-width:110px;text-align:center;">'+a.name+'<div class="small muted" style="font-weight:400">points / max</div></th>'; });
    hr += '</tr>';
    thead.innerHTML=hr;
    ...
    STUDENTS.forEach((s,ri)=>{
      const tr = el("tr"); tr.style.borderTop="1.6px dashed var(--line-soft)";
      let html = '<td style="padding:9px 14px;position:sticky;left:0;background:var(--panel);"><div style="display:flex;align-items:center;gap:9px;"><span class="avatar">'+s.init+'</span><span style="font-size:16px;font-weight:700;white-space:nowrap;">'+s.name+'</span></div></td>';
      tr.innerHTML=html;
      VIEW_ASGS.forEach((a,ci)=>{
        const td = el("td"); td.style.cssText="padding:9px 10px;text-align:center;";
        const v = state.scores[key(s.id,a.id)];
        const abs = v==="abs";
        const cell = el("div","scell"); cell.style.justifyContent="center";
        const inp = el("input","sin"+(abs?" abs":"")); inp.type="text"; inp.inputMode="numeric";
        inp.value = abs? "Abs" : (v!=null?v:"");
        inp.dataset.r=ri; inp.dataset.c=ci; inp.dataset.s=s.id; inp.dataset.a=a.id;
        const den = el("span","den","/"+(s.max[a.id]??s.defaultMax));
        cell.appendChild(inp); cell.appendChild(den);
        addCellNote(cell,inp,key(s.id,a.id));
        const wrapA=el("div"); wrapA.style.cssText="display:flex;flex-direction:column;align-items:center;gap:4px;";
        wrapA.appendChild(cell); wrapA.insertAdjacentHTML("beforeend",lvlChip(s.lvl[a.id]??s.defaultLvl));
        td.appendChild(wrapA); tr.appendChild(td);
      });
    ...
    keys.innerHTML =
      '<span><b>Type</b> a number to score</span>'+
      '<span><span class="kbd">Enter</span> next student ↓</span>'+
      '<span><span class="kbd">Tab</span> prompting for this cell</span>'+
      '<span><span class="kbd">↑↓←→</span> move</span>'+
      '<span><span class="kbd">A</span> absent</span>'+
      '<span class="muted">· autosaves as you go</span>';
    root.appendChild(keys);

    // annotations
    root.appendChild(anno("64px","auto","One screen = the whole stack. Columns are assignments, rows are kids."+ARRR));
    $(".anno:last-child",root).style.right="14px"; $(".anno:last-child",root).style.left="auto";
    root.appendChild(anno("auto","16px","Denominator is pre-set per student — teacher only types the points correct, never the max."));
    $(".anno:last-child",root).style.top="232px";
    root.appendChild(anno("auto","16px","Enter blasts numbers down the column. Tab on any cell sets the prompting level — the next stop only if you want it, never a detour."+ARR));
    const a3=$(".anno:last-child",root); a3.style.bottom="62px"; a3.style.left="auto"; a3.style.right="18px";

    // wiring
    wireGridInputs(root,'down');
  }

  function wireGridInputs(root, enterDir){
    const inputs = [...root.querySelectorAll(".sin")];
    const grid = {}; let maxR=0, maxC=0;
    inputs.forEach(i=>{ const r=+i.dataset.r,c=+i.dataset.c; grid[r+"-"+c]=i; if(r>maxR)maxR=r; if(c>maxC)maxC=c; });
    function focusCell(r,c){ const t=grid[r+"-"+c]; if(t){ t.focus(); t.select&&t.select(); } return !!t; }
    function fwd(r,c){ if(enterDir==='right'){ if(!focusCell(r,c+1)) focusCell(r+1,0); } else { if(!focusCell(r+1,c)) focusCell(0,c+1); } }
    function back(r,c){ if(enterDir==='right'){ if(!focusCell(r,c-1)) focusCell(r-1,maxC); } else { if(!focusCell(r-1,c)) focusCell(maxR,c-1); } }
    inputs.forEach(inp=>{
      const r=+inp.dataset.r, c=+inp.dataset.c;
      const dot=inp.closest(".scell").querySelector(".cellnote");
      inp.addEventListener("focus",()=>inp.closest(".scell").classList.add("focused"));
      inp.addEventListener("blur",()=>inp.closest(".scell").classList.remove("focused"));
      inp.addEventListener("input",()=>{
        const val=inp.value.trim();
        state.scores[key(inp.dataset.s,inp.dataset.a)] = val===""?"":val.replace(/[^0-9]/g,"");
        inp.classList.remove("abs");
      });
      inp.addEventListener("keydown",e=>{
        if(e.key==="Enter"){ e.preventDefault(); fwd(r,c); }
        else if(e.key==="Tab"){ e.preventDefault(); if(e.shiftKey){ back(r,c); } else { openCellPopover(inp,dot,()=>fwd(r,c)); } }
        else if(e.key==="ArrowDown"){ e.preventDefault(); focusCell(r+1,c); }
        else if(e.key==="ArrowUp"){ e.preventDefault(); focusCell(r-1,c); }
        else if(e.key==="ArrowRight"){ if(inp.selectionStart>=inp.value.length){ e.preventDefault(); focusCell(r,c+1);} }
        else if(e.key==="ArrowLeft"){ if(inp.selectionStart<=0){ e.preventDefault(); focusCell(r,c-1);} }
        else if(e.key==="a"||e.key==="A"){ e.preventDefault();
          state.scores[key(inp.dataset.s,inp.dataset.a)]="abs";
          inp.value="Abs"; inp.classList.add("abs"); fwd(r,c);
        }
        else if(e.key==="n"||e.key==="N"){ e.preventDefault(); openCellPopover(inp,dot,()=>fwd(r,c)); }
      });
    });
  }

  // ============================================================
  // DIRECTION D — FLIPPED GRID (assignments = rows, students = columns)
  // ============================================================
  function renderD(){
    const root=$("#dirD"); root.innerHTML=""; root.style.position="relative";

    const ctx=el("div","ctx");
    ctx.innerHTML='<div class="field"><span class="lab">Assignments</span><div class="chips">'+
      ASSIGNMENTS.map(a=>'<span class="chip on">'+a.name+' <span class="x">×</span></span>').join('')+
      '<span class="chip add">+ add</span></div></div>';
    const pairs=[]; STUDENTS.forEach(s=>ASSIGNMENTS.forEach(a=>pairs.push([s.id,a.id])));
    ctx.appendChild(progressEl(countDone(pairs),pairs.length,"scores"));
    root.appendChild(ctx);
    ...
    const table=el("table"); table.style.cssText="border-collapse:collapse;width:100%;font-family:'Kalam',cursive;";
    const thead=el("thead");
    let hr='<tr><th style="text-align:left;padding:12px 14px;position:sticky;left:0;background:var(--panel);font-size:15px;min-width:140px;z-index:2;">Assignment</th>';
    STUDENTS.forEach(s=>{ hr+='<th style="padding:10px 6px;min-width:92px;text-align:center;vertical-align:bottom;"><div style="display:flex;flex-direction:column;align-items:center;gap:5px;"><span class="avatar">'+s.init+'</span><span style="font-size:13px;font-weight:700;white-space:nowrap;">'+s.name+'</span></div></th>'; });
    hr+='</tr>'; thead.innerHTML=hr; table.appendChild(thead);
    const tbody=el("tbody");
    VIEW_ASGS.forEach((a,ri)=>{
      const tr=el("tr"); tr.style.borderTop="1.6px dashed var(--line-soft)";
      tr.innerHTML='<td style="padding:12px 14px;position:sticky;left:0;background:var(--panel);font-size:16px;font-weight:700;z-index:1;">'+a.name+'<div class="small muted" style="font-weight:400">points / max</div></td>';
      STUDENTS.forEach((s,ci)=>{
        const td=el("td"); td.style.cssText="padding:9px 6px;text-align:center;";
        const v=state.scores[key(s.id,a.id)]; const abs=v==="abs";
        const cell=el("div","scell"); cell.style.justifyContent="center";
        const inp=el("input","sin"+(abs?" abs":"")); inp.type="text"; inp.inputMode="numeric";
        inp.value=abs?"Abs":(v!=null?v:"");
        inp.dataset.r=ri; inp.dataset.c=ci; inp.dataset.s=s.id; inp.dataset.a=a.id;
        ...
        cell.appendChild(inp); cell.appendChild(den);
        addCellNote(cell,inp,key(s.id,a.id));
        const wrapD=el("div"); wrapD.style.cssText="display:flex;flex-direction:column;align-items:center;gap:4px;";
        wrapD.appendChild(cell); wrapD.insertAdjacentHTML("beforeend",lvlChip(s.lvl[a.id]??s.defaultLvl));
        td.appendChild(wrapD); tr.appendChild(td);
      });
      tbody.appendChild(tr);
    });
    ...
    root.appendChild(anno("60px","auto","Flip it when you\u2019ve just graded one assignment for the whole class — fill straight across the row."));
    const d1=$(".anno:last-child",root); d1.style.top="58px"; d1.style.right="14px"; d1.style.left="auto";
    root.appendChild(anno("auto","16px","Same flow as the other grid — Enter runs across the class, Tab sets the prompting level for any cell. Just pivoted."));
    const d2=$(".anno:last-child",root); d2.style.bottom="62px"; d2.style.left="18px";

    wireGridInputs(root,'right');
  }

  // ============================================================
  // DIRECTION B — WORKSHEET STACK (one-up card flow)
  // ============================================================
  function buildQueue(){
    const q=[]; ASSIGNMENTS.forEach(a=>STUDENTS.forEach(s=>q.push({s,a}))); return q;
  }
  function renderB(){
    const root=$("#dirB"); root.innerHTML=""; root.style.position="relative";
    const q=buildQueue();
    const i=Math.min(state.stackI,q.length-1);
    const cur=q[i];

    const ctx=el("div","ctx");
    ctx.innerHTML='<div class="field"><span class="lab">Scoring queue</span><div class="select rough soft tight">All worksheets <span class="car">▾</span></div></div>';
    ctx.appendChild(progressEl(i, q.length, "worksheets"));
    root.appendChild(ctx);

    // stage with ghost stack + current card
    const wrap=el("div"); wrap.style.cssText="display:flex;justify-content:center;padding:18px 0 8px;";
    const deck=el("div"); deck.style.cssText="position:relative;width:540px;max-width:100%;";
    // ghost cards behind
    deck.innerHTML =
      '<div class="rough soft" style="position:absolute;inset:0;transform:translate(16px,16px) rotate(1.4deg);background:var(--panel);border-radius:14px;height:360px;"></div>'+
      '<div class="rough soft" style="position:absolute;inset:0;transform:translate(8px,8px) rotate(-1deg);background:var(--panel);border-radius:14px;height:360px;"></div>';
    const card=el("div","rough"); card.style.cssText="position:relative;background:var(--panel);border-radius:14px;padding:22px 24px;min-height:360px;";
    const k=key(cur.s.id,cur.a.id);
    const v=state.scores[k];
    card.innerHTML =
      '<div style="display:flex;justify-content:space-between;align-items:flex-start;">'+
        '<div><div class="muted small" style="text-transform:uppercase;letter-spacing:1px;">Worksheet '+(i+1)+'</div>'+
        '<div style="display:flex;align-items:center;gap:11px;margin-top:6px;"><span class="avatar" style="width:38px;height:38px;font-size:15px;">'+cur.s.init+'</span><div style="display:flex;flex-direction:column;gap:4px;"><b style="font-size:23px;line-height:1;">'+cur.s.name+'</b>'+lvlChip(cur.s.lvl[cur.a.id]??cur.s.defaultLvl)+'</div></div>'+
        '<div style="font-size:17px;margin-top:4px;">'+cur.a.name+'</div></div>'+
        '<button class="btn ghost sm" id="bAbsent">Mark absent</button>'+
      '</div>'+
      '<div style="display:flex;align-items:baseline;gap:10px;justify-content:center;margin:22px 0 6px;">'+
        '<input class="sin" id="bScore" style="width:96px;height:74px;font-size:42px;border-width:2px;border-color:var(--line);" inputmode="numeric" value="'+(v&&v!=="abs"?v:"")+'" placeholder="–">'+
        '<span style="font-size:30px;color:var(--ink3);font-weight:700;">/ '+cur.s.max[cur.a.id]+'</span>'+
      '</div>'+
      '<div class="muted small" style="text-align:center;">points correct</div>'+
      '<div class="hr"></div>'+
      '<div class="lab" style="font-size:13px;color:var(--ink2);font-weight:700;margin-bottom:6px;">How did it go? <span class="muted" style="font-weight:400">— optional, one tap</span></div>'+
      '<div class="chips" data-grp="prompt" style="margin-bottom:10px;">'+PROMPTS.map(p=>'<span class="chip'+(state.prompt[k]===p?' on':'')+'" data-v="'+p+'">'+p+'</span>').join('')+'</div>'+
      '<div class="chips" data-grp="accom" style="margin-bottom:12px;">'+ACCOMS.map(p=>'<span class="chip add" data-v="'+p+'">+ '+p+'</span>').join('')+'</div>'+
      '<input class="rough soft" id="bNote" style="width:100%;background:var(--shade);border:none;border-radius:9px;padding:9px 12px;font-family:Kalam,cursive;font-size:15px;" placeholder="quick note…" value="'+(state.note[k]||"")+'">'+
      '<div style="display:flex;gap:12px;margin-top:16px;justify-content:flex-end;align-items:center;">'+
        '<button class="btn ghost" id="bSkip">Skip</button>'+
        '<button class="btn primary" id="bNext">Save &amp; next ▸</button>'+
      '</div>';
    deck.appendChild(card);
    wrap.appendChild(deck);
    root.appendChild(wrap);

    const keys=el("div","keys"); keys.style.justifyContent="center";
    keys.innerHTML='<span><b>Type</b> the score</span><span><span class="kbd">Enter</span> save &amp; next worksheet</span><span><span class="kbd">←</span><span class="kbd">→</span> flip through the stack</span>';
    root.appendChild(keys);

    // annotations
    root.appendChild(anno("96px","16px","Mirrors the physical pile: one worksheet up at a time, the rest stacked behind."+ARR));
    root.appendChild(anno("auto","auto","Single focus → room to capture the rich stuff (prompting, note) without slowing the run."));
    const an2=$(".anno:last-child",root); an2.style.top="300px"; an2.style.right="14px"; an2.style.left="auto"; an2.innerHTML+=ARRR;

    // wiring
    const adv=(d)=>{ state.stackI=Math.max(0,Math.min(q.length-1,i+d)); renderB(); requestAnimationFrame(()=>{const f=$("#bScore");f&&f.focus();}); };
    const sc=$("#bScore",card);
    sc.addEventListener("input",()=>{ state.scores[k]=sc.value.replace(/[^0-9]/g,""); });
    sc.addEventListener("keydown",e=>{ if(e.key==="Enter"){e.preventDefault();adv(1);} else if(e.key==="ArrowRight"&&sc.selectionStart===sc.value.length){adv(1);} else if(e.key==="ArrowLeft"&&sc.selectionStart===0){adv(-1);} });
    $("#bNext",card).addEventListener("click",()=>adv(1));
    $("#bSkip",card).addEventListener("click",()=>adv(1));
    $("#bAbsent",card).addEventListener("click",()=>{ state.scores[k]="abs"; adv(1); });
    $("#bNote",card).addEventListener("input",e=>{ state.note[k]=e.target.value; });
    card.querySelectorAll('[data-grp="prompt"] .chip').forEach(ch=>ch.addEventListener("click",()=>{
      state.prompt[k]= state.prompt[k]===ch.dataset.v?null:ch.dataset.v;
      card.querySelectorAll('[data-grp="prompt"] .chip').forEach(c=>c.classList.toggle("on",c.dataset.v===state.prompt[k]));
    }));
    card.querySelectorAll('[data-grp="accom"] .chip').forEach(ch=>ch.addEventListener("click",()=>ch.classList.toggle("on")));
    requestAnimationFrame(()=>{ if(state.dir==="B"){const f=$("#bScore");f&&f.focus();} });
  }
  ...
  // shell wiring
  // ============================================================
  const LEAF={A:"Enter scores · grid",B:"Enter scores · stack",C:"Enter scores · roster (wide)",E:"Enter scores · roster (narrow)",D:"Enter scores · grid (flipped)"};
  function renderCurrent(){ ({A:renderA,B:renderB,C:renderC,E:renderE,D:renderD})[state.dir](); }
  ...
  function switchTab(dir){
    ...
    ["A","B","C","E","D"].forEach(d=>$("#dir"+d).classList.toggle("active",d===dir));
